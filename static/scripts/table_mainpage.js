...
mainpage = {
    ...
    showPreviousPage: function showPreviousPage() {
        if (Start === 0) {
            ...
            End -= 15;
            this.deleteRows();
            this.createTable(shows, Start, End)
        }
    },
    ...
    createRow: function createRow(show) {
        ...
        row.innerHTML = '<td>' + this.addLinkToShowTitle(show.id, show.title) + '</td>' +
            ...
            '<td>' + this.seasonsHandler(show.seasons,show) + '</td>' +
    ...
    createSeasonsModalUnderButton: function createSeasonsModalUnderButton(show ){
        ...
        document.getElementById("button"+show.id).addEventListener("click", function() {
                ...
                this.createHeaderSeasonsModal(showTitle);
                this.createTableSeasonsModal(showSeasons, tableBodySeasons);
